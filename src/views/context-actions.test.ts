...
test("shared header keeps one system bar and no legacy navigation rows", () => {
  ...
  assert.match(read("src/views/partials/footer.ejs"), /class="container rm-page-shell"/);
  ...
  assert.doesNotMatch(css, /--rm-page-shell-max-width/);
  assert.doesNotMatch(css, /1320px|1040px/);
  assert.doesNotMatch(css, /--rm-page-shell-edge-space/);
...
test("shell-scoped overflow and wrapper behavior stays consistent", () => {
  ...
  assert.match(css, /\.customers-table-wrapper,\s*\.staff-table-wrapper,\s*\.vehicle-table-wrapper,\s*\.table-wrapper \{[\s\S]*width: 100%;[\s\S]*min-width: 0;[\s\S]*overflow-x: auto;/);
